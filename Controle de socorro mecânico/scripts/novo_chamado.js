...
salvarChamado.addEventListener("click", () => {
  ...
  const novoChamado = new Chamado({
    ...
    problema: document.querySelector("select").options[document.querySelector("select").selectedIndex].text,
...
function atualizarResumo() {
  ...
  const problema = select?.value ? select.options[select.selectedIndex].text : "—";

  const endereco = [pegar("rua"), pegar("numero")].filter((v) => v !== "—").join(", ") || "—";

  const equipeSelecionada = equipe.find((el) => el.classList.contains("equipe_selecionado"));
  const nomeEquipe = equipeSelecionada?.querySelector(".nome_funcionario")?.textContent.trim() || "Ainda não Atribuída";
...
document.querySelector("select").addEventListener("change", (e) => {
  document.getElementById("inf_problema").textContent = e.target.options[e.target.selectedIndex].text;
});

document.querySelectorAll(".botoes_prioridade > button").forEach((botao) => {
  botao.addEventListener("click", () => {
    document.querySelectorAll(".botoes_prioridade > button").forEach((b) => b.classList.remove("selecionado"));
    botao.classList.add("selecionado");

    const prioridade = Object.keys(cores).find((c) => botao.classList.contains(c));
    if (prioridade) {
      divPrioridade.textContent = botao.textContent;
...
camposEndereco.forEach((id) => {
  document.getElementById(id).addEventListener("input", () => {
    ...
    document.getElementById("inf_endereco").textContent = [rua, numero].filter(Boolean).join(", ") || "—";
  });
});
